...
class ShopDashboardManager {
    ...
    async init() {
        ...
        // Set default dates (Month) to match UI label
        this.applyPresetFilter('month');
    ...
    async loadOutstandingCredit() {
        try {
            // Get credits for shop filtered by date range
            ...
            const { data: credits, error } = await creditQuery;

            if (error) throw error;

            // Calculate filtered total credit and pending balance
            const totalCreditFiltered = credits?.reduce((sum, credit) => sum + parseFloat(credit.total_amount || 0), 0) || 0;

            // Note: Pending Balance is usually a CURRENT status, but the user asked for the "card" by filter.
            // Usually, "Pending" also follows the date of the credit.
            const totalPendingFiltered = credits?.reduce((sum, credit) => sum + parseFloat(credit.pending_amount || 0), 0) || 0;

            this.totalOutstandingCredit = totalPendingFiltered;

            if (document.getElementById('totalCredit')) {
                document.getElementById('totalCredit').textContent = formatCurrency(totalCreditFiltered);
            }

            // Update the change/label for Total Credit card if it exists
            const creditLabel = document.querySelector('#totalCredit + .change');
            if (creditLabel) {
                creditLabel.textContent = this.currentFilter === 'all' ? 'All Time' :
                    this.currentFilter.charAt(0).toUpperCase() + this.currentFilter.slice(1);
            }

            if (document.getElementById('pendingBalance')) {
                document.getElementById('pendingBalance').textContent = formatCurrency(totalPendingFiltered);
            }
        ...
        try {
            ...
            for (let i = 0; i < range.length; i++) {
                ...
                try {
                    ...
                    const { data: dailySalesRaw, error } = await supabaseClient
                        .from('sales')
                    ...
                    const dailySales = dailySalesRaw ? dailySalesRaw.filter(sale => sale.sale_status !== 'refunded') : [];
    ...
    async loadRecentSales() {
        try {
            ...
            const sales = salesRaw ? salesRaw.filter(sale => sale.sale_status !== 'refunded').slice(0, 10) : [];
    ...
    async loadFinancialSummaries() {
        try {
            ...
            // C. All-Time Pending Credits
            const { data: allCredits } = await supabaseClient.from('credits')
                .select('pending_amount')
                .eq('shop_id', this.shopId);
            const allTimePending = allCredits?.reduce((sum, c) => sum + parseFloat(c.pending_amount || 0), 0) || 0;
            ...
            // valuation formula: (Sales + CashInHand + InventorySellValue) - Expenses
            const valuation = (allTimeSales + actualBalance + inventorySellValue) - allTimeExp;
