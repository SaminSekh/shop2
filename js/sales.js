...
class SalesManager {
    ...
    updateSalesStats() {
        ...
        const periodMargin = periodTotal > 0 ? (periodProfit / periodTotal * 100) : 0;
        ...
        if (marginChangeEl) {
            ...
            const yesterdayMargin = yesterdayTotal > 0 ? (yesterdayProfit / yesterdayTotal * 100) : 0;
                ...
                <tr class="${isSelected ? 'selected-row' : ''}">
                    ...
                    <td>
                        <div class="d-flex gap-1">
                            ...
                            ` : ''}
                            <button class="btn btn-sm btn-danger delete-sale-btn" data-id="${sale.id}" title="Move to Trash & Restock">
    ...
    async handleDelete(saleId) {
        if (!confirm('Are you sure you want to delete this sale record? This will permanently remove the record and restore product stock.')) {
            return;
        }
        ...
        try {
            ...
            showNotification('Sale record deleted and stock restored', 'success');
            await this.loadSalesData();
        } catch (error) {
            console.error('Error deleting sale:', error);
            showNotification('Failed to delete sale: ' + (error.message || error), 'error');
        } finally {
            showLoading(false);
        }
    }
    ...
    async handleRefund(saleId) {
        ...
        try {
            ...
            showNotification('Sale marked as refunded and items restocked', 'success');
            await this.loadSalesData();
        } catch (error) {
