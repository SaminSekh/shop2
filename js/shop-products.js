...
class ShopProductsViewer {
    ...
    async loadSystemDomains() {
        try {
            ...
            // Domain Enforcement: If on mgmt domain but viewing a shop, redirect to public domain
            ...
            // Update UI after loading domains
            this.updateUI();
        } catch (e) {
        ...
        grid.innerHTML = this.filteredProducts.map(product => {
            ...
            if (metadata && typeof metadata === 'object') {
                ...
                for (const [key, val] of Object.entries(metadata)) {
                    // Skip image-related keys and system fields
                    if (val && key !== 'product_images' && key !== 'product_image') {
            ...
            return `
                <div class="public-product-card">
                    <div class="public-product-img" onclick="app.openProductDetail('${product.id}')">
                        <img src="${this.getAssetUrl(product.product_image || 'assets/default-product.png')}" alt="${product.product_name}" style="cursor: zoom-in;">
                    </div>
    ...
    openProductDetail(productId) {
        ...
        if (modal && img) {
            img.src = this.getAssetUrl(product.product_image || 'assets/default-product.png');
            ...
            if (metadata && typeof metadata === 'object') {
                for (const [key, val] of Object.entries(metadata)) {
                    if (val && key !== 'product_images' && key !== 'product_image') { // Skip product images in specs
                        const label = key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, ' $1');

                        // Smart Icon
                        let iconClass = 'fas fa-check-circle';
                        const keyLower = key.toLowerCase();
                        if (keyLower.includes('date') || keyLower.includes('expiry')) iconClass = 'fas fa-calendar-alt';
                        else if (keyLower.includes('time')) iconClass = 'fas fa-clock';
                        else if (keyLower.includes('weight') || keyLower.includes('unit') || keyLower.includes('volume')) iconClass = 'fas fa-weight-hanging';
                        else if (keyLower.includes('size') || keyLower.includes('dimension')) iconClass = 'fas fa-ruler-combined';
                        else if (keyLower.includes('material')) iconClass = 'fas fa-layer-group';
                        else if (keyLower.includes('color')) iconClass = 'fas fa-palette';
                        else if (keyLower.includes('brand')) iconClass = 'fas fa-trademark';
